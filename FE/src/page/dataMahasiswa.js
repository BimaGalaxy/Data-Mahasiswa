...
function DataMahasiswa() {
    ...
    const deleteMahasiswa = async (mhsID) => {
        try {
            await axios.delete(`http://localhost:8000/mahasiswa/${mhsID}`);
            getAllMahasiswa();
        } catch (error) {
            console.log(error);
        }
    ...
    const getAllMahasiswa = async () => {
        const respon = await axios.get("http://localhost:8000/mahasiswa");
    ...
    const handleSearch = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.get(`http://localhost:8000/mahasiswa/search/${searchQuery}`);
